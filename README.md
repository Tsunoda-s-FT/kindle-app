...
Kindle WebリーダーまたはKindle macOSアプリから自動的にスクリーンショットを取得し、PDFを生成するツールです。

## 特徴

- 全ページを自動キャプチャ（KindleRenderer API使用）
- Kindle macOSアプリの自動キャプチャ（osascript/screencapture）
- シングル/見開きレイアウト対応
- PDF生成（リサイズ・品質調整可能）
- 進捗表示とメタデータ記録
...
- **Kindle macOSアプリ** (ローカルキャプチャ時)
- **画面収録/アクセシビリティ許可** (ローカルキャプチャ時)
...
### Kindle macOSアプリからキャプチャ（任意）

1. Kindleアプリで対象の本を開く
2. 画面収録/アクセシビリティの許可を与える（初回のみ）
3. スクリーンショット取得

```bash
python src/capture_app.py --book "My Book"
```

4. PDF生成

```bash
python src/create_pdf.py --input "./kindle-captures/My Book/"
```
...
|----------|------|-----------|
| `--book` | 書籍名/識別子（必須） | - |
| `--output` | 出力ディレクトリ | ./kindle-captures/{book}/ |
| `--app-name` | アプリ名 | Amazon Kindle |
| `--process-name` | プロセス名 | Kindle |
| `--window-title` | ウィンドウ名の部分一致 | - |
| `--region` | キャプチャ領域 `x,y,width,height` | - |
| `--scale` | 座標スケール（Retinaなら2.0） | 1.0 |
| `--wait` | ページ送り後の待機秒数 | 0.6 |
| `--max-pages` | 取得ページ数の上限 | 無制限 |
| `--dup-threshold` | 重複判定の距離 | 3 |
| `--dup-diff-mean` | 重複判定の平均差分 | 3.0 |
| `--dup-size-kb` | 重複判定のサイズ差(KB) | - |
| `--dup-size-ratio` | 重複判定のサイズ差率 | 0.02 |
| `--dup-limit` | 連続重複の停止回数 | 5 |
| `--min-pages` | 重複停止の最小ページ数 | 2 |
| `--next-key` | ページ送りキー | right |
| `--log-file` | ログ出力先 | - |
...
```bash
# 20ページだけ取得
python src/capture_app.py --book "My Book" --max-pages 20

# 領域指定でキャプチャ
python src/capture_app.py --book "My Book" --region "120,90,1800,2400"
```

※ UIの余白を取り除く場合は `trim.py` の利用が便利です。
...
app_capture:
  ...
  process_name: "Kindle"
  window_title: null
  capture_region: null  # "x,y,width,height"
  scale: 1.0
  output_dir: "./kindle-captures"
  wait_after_turn: 0.6
  initial_wait: 0.4
  ...
  duplicate_size_kb: null
  duplicate_size_ratio: 0.02
  duplicate_limit: 5
  min_pages: 2
  next_key: "right"
...
├── src/
│   ├── __init__.py
│   ├── capture.py           # Webスクリーンショット取得
│   ├── capture_app.py       # macOSアプリのスクリーンショット取得
│   ├── create_pdf.py         # PDF生成
...
※ Kindle macOSアプリのキャプチャでは `source: "app"` や `capture_region` などが記録されます。
...
### 画面収録/アクセシビリティが許可されていない（ローカル）

**症状**: `screencapture` が失敗する / `osascript` の権限エラー

**対処**:
1. macOS「プライバシーとセキュリティ」で「画面収録」を許可
2. 「アクセシビリティ」でターミナル（またはpython実行元）を許可
...
- **osascript/screencapture**: macOSアプリのキャプチャ
...
- **Chrome必須**（他のブラウザは非対応）
- **ログイン状態必須**（自動ログインは未実装）
- **ローカルはページ数取得不可**（重複判定か`--max-pages`で終了）
