...
- トリミング範囲（絶対座標）
- PIL形式のcrop座標：左上(left,top)と右下(right,bottom)
- 例: `--crop "<left,top,right,bottom>"`（画像から測定した値）
...
- トリミング後の画像の出力先
- デフォルト: `{input}/trimmed/`
- 例: `--output ./kindle-captures/B0DSKPTJM5/custom_trimmed/`

**`--note <テキスト>`**
- trim_metadata.jsonに記録するメモ
- トリミング履歴の追跡に使用
- 例: `--note "左右マージンを調整"`
...
# カスタム出力先
python src/trim.py --input ./kindle-captures/B0DSKPTJM5/ --crop "<left,top,right,bottom>" --output ./trimmed_output/

# メモ付き（履歴追跡用）
python src/trim.py --input ./kindle-captures/B0DSKPTJM5/ --crop "<left2,top,right2,bottom>" --note "2回目：マージン縮小"
...
AIエージェントが画像を確認して決定します：

1. **元画像のサイズを確認**: viewport設定に依存するため、実画像のサイズを参照
2. **本体ページの外枠を特定**:
   - 上端/下端/左右端を視覚的に特定
   - 見開きの場合は左ページ外枠の左端と右ページ外枠の右端を揃える
3. **UIは無視**: ヘッダー/Location/ナビ矢印は判断材料にしない
4. **外枠を欠かない範囲で調整**: 本体外枠が欠けない最小限の余裕を確保

**注意**: 数値の例は記載しません。必ず対象画像を見て測定し、他書籍の値を流用しないこと。

#### 出力ファイル

**trimmed/ ディレクトリ**:
- 元画像と同じファイル名（page_0001.png, page_0002.png, ...）
- 全画像が同一サイズにトリミング
...
```json
{
  ...
  "trimmed_size": {"width": "<width>", "height": "<height>"},
  "total_pages": 245,
  "trimmed_at": "2025-01-15T10:30:45.123456",
  "note": "初回トリミング",
  "history": []
}
```
