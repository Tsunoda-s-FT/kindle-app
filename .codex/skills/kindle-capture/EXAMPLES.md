...
**ユーザー**: この本のPDFを作成して、余計な余白を除去してください
...
```bash
python src/capture.py --asin B0FQHGKTZ1 --layout double
```

（キャプチャ実行中...）

キャプチャが完了しました（245ページ）。サンプル画像を確認してトリミング範囲を決定します。

（AIがpage_0001.png, page_0003.png, page_0005.pngを確認）

画像を確認しました。本体ページの外枠（上/下/左右）を特定しました。UIは無視します。

推奨トリミング範囲: `"<left,top,right,bottom>"`（画像から測定した値）

トリミングを実行します。

```bash
python src/trim.py --input ./kindle-captures/B0FQHGKTZ1/ --crop "<left,top,right,bottom>"
```

（トリミング実行中...）

トリミングが完了しました。結果を確認します。

（AIがtrimmed/page_0001.png, trimmed/page_0005.pngを確認）

トリミング結果を確認しました。本体外枠に沿ってトリミングされ、コンテンツは正しく保持されています。PDFを生成します。

```bash
python src/create_pdf.py --input ./kindle-captures/B0FQHGKTZ1/trimmed/
```

本体外枠に合わせたPDFファイル `B0FQHGKTZ1.pdf` を生成しました。

---

### 例7: トリミングの調整・やり直し

**ユーザー**: トリミングが少し切りすぎているので調整してください

**AIアシスタント**: 承知しました。現在の設定を確認します。

（AIがtrimmed/trim_metadata.jsonを確認）

現在のcrop_box: `"<left,top,right,bottom>"`

トリミング後の画像を確認します。

（AIがtrimmed/page_0004.pngを確認）

確認しました。左右の本体外枠が少し切れているようです。より保守的な設定でやり直します。

```bash
python src/trim.py --input ./kindle-captures/B0FQHGKTZ1/ --crop "<left2,top,right2,bottom>" --note "左右マージン拡大"
```

（トリミング実行中...元画像から再トリミング）

やり直しが完了しました。結果を確認します。

（AIがtrimmed/page_0004.pngを確認）

今回は本体外枠が適切に収まっています。PDFを再生成します。
